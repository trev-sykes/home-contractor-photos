...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="flex h-screen overflow-hidden" style={{ backgroundColor: "var(--color-bg)" }}>
            ...
            {/* ── SIDEBAR (md+) ── */}
            <aside
                className="hidden md:flex w-64 flex-shrink-0 flex-col p-5 h-screen"
                style={{
            ...
            >
                {/* Logo */}
                <Link href="/dashboard" className="flex items-center gap-3 px-2 mb-8 mt-1 flex-shrink-0">
                    <div
                ...
                <p className="text-xs font-bold uppercase tracking-widest px-3 mb-3 flex-shrink-0" style={{ color: "var(--color-text-faint)" }}>
                    Menu
                </p>
                ...
                <div className="flex-shrink-0 pt-4 mt-4" style={{ borderTop: "1px solid var(--color-border-subtle)" }}>
                    <button
            ...
            {/* ── MAIN ── */}
            <main className="flex-1 overflow-y-auto pb-20 md:pb-0" style={{ backgroundColor: "#f1f5f9" }}>
                ...
                {navLinks.map(({ href, label, icon: Icon }) => {
                    ...
                    return (
                        <Link
                            key={href}
                            href={href}
                            className="flex flex-col items-center gap-0.5 px-2 py-1 rounded-xl transition flex-1"
                            style={{ color: active ? "var(--color-amber)" : "var(--color-text-faint)" }}
                        >
